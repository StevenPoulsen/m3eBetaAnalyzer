...
export class CrewBuilderService {
  ...
  constructor(private dataService: DataService, private menuService: MenuService, private ea: EventAggregator, private dialogService: DialogService) {
    const cacheString:string = localStorage.getItem("printCrew");
    if (cacheString) {
      this.currentCrew = JSON.parse(cacheString);
    }
  }
  ...
  get hasChanges(): boolean {
    return !!this.currentCrew && !!this.lastModified && (!this.currentCrew.lastSave || this.lastModified > this.currentCrew.lastSave);
  }
  ...
  public isLegal(model) {
    ...
    return model.factions.indexOf(this.currentCrew.faction) > -1;
  }
  ...
  public saveCrew(saveName:string): void {
    ...
    localStorage.setItem(this.cacheKey(), JSON.stringify(crews));
    this.lastModified = new Date().getTime();
  ...
  public deleteCrew(saveName: string): void {
    const crews = this.getCrews();
    for (let i = 0, len = crews.length, crew; i < len; i++) {
      crew = crews[i];
      if (crew.saveName === saveName) {
